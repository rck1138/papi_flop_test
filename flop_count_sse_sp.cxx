...
int main(int argc, char **argv)
{
	...
	long long cvals;
	rtype accum = 0.0;
   rtype *a1 = (rtype *) _mm_malloc(N*sizeof(rtype),16);
	rtype *a2 = (rtype *) _mm_malloc(N*sizeof(rtype),16);
	rtype *a3 = (rtype *) _mm_malloc(N*sizeof(rtype),16);

   vtype *a1v = (vtype *) _mm_malloc((N/vlen)*sizeof(vtype),16);
   vtype *a2v = (vtype *) _mm_malloc((N/vlen)*sizeof(vtype),16);
   vtype *a3v = (vtype *) _mm_malloc((N/vlen)*sizeof(vtype),16);
   vtype phi = _mm_set1_ps(1.618033);
   ...
   for(int i=0; i<N/vlen; i++){
      a1v[i] = _mm_load_ps(&(a1[vlen*i]));
      a2v[i] = _mm_load_ps(&(a2[vlen*i]));
   }
...
#endif
      a3v[i] = _mm_add_ps(a1v[i], _mm_mul_ps(phi,a2v[i]));
	} 
   ...
   for(int i=0; i<N/vlen; i++) _mm_store_ps (&a3[vlen*i], a3v[i]);
